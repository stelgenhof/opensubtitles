...
try {
    ...
    if (null === $response) {
        ...
        $response = $client->call('LogIn', [
            \getenv('OPENSUBTITLES_USERNAME'),
            \getenv('OPENSUBTITLES_PASSWORD'),
            'en',
            \getenv('OPENSUBTITLES_USER_AGENT')
        ]);
        ...
        if ('200 OK' === $response['status'] && ! empty($response['token'])) {
            $response = $client->call(
                'SearchSubtitles',
                [$response['token'], [['sublanguageid' => \getenv('OPENSUBTITLES_LANGUAGES'), 'imdbid' => $imdbID]]]
            );
